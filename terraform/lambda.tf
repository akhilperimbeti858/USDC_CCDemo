locals {
  ...
  post_merge_function_name  = "${var.project_name}-SageMaker-post-merge"
}
...
data "archive_file" "post_merge" {
  type        = "zip"
  source_dir  = "${path.module}/../lambdas/post_annotation_merge"
  output_path = "${path.module}/build/post_annotation_merge.zip"
}
...
resource "aws_lambda_function" "post_merge" {
  function_name    = local.post_merge_function_name
  role             = aws_iam_role.post_lambda.arn
  handler          = "handler.lambda_handler"
  runtime          = "python3.12"
  timeout          = 300
  filename         = data.archive_file.post_merge.output_path
  source_code_hash = data.archive_file.post_merge.output_base64sha256
}
...
resource "aws_lambda_permission" "gt_invoke_post_merge" {
  statement_id  = "AllowGroundTruthInvokePostMerge"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.post_merge.function_name
  principal     = "sagemaker.amazonaws.com"
}
